...
// <InvoicePrintButton 
//   saleId={vente.id}
//   saleNumber={vente.sale_number}
//   currentUser={currentUser} // ← Ajoutez currentUser
//   saleConfig={saleConfig} // ← Ajoutez saleConfig
//   onPrintComplete={() => {
//     // Optionnel: recharger les données après impression
//     loadTodaysSales()
//   }}
// />                                 {/* <button
...
export default function VentesDashboard() {
  ...
  // Ventes du jour pour l'utilisateur connecté
  const [todaysSales, setTodaysSales] = useState([])
  const [todaysOutflows, setTodaysOutflows] = useState([])
  ...
  const loadTodaysOutflows = async () => {
    if (!currentUser) return
  ...
  const loadDailyStats = async () => {
    ...
    try {
      ...
      // Récupérer toutes les sorties du jour pour l'utilisateur
      ...
      if (outflows) {
        outflows.forEach(outflow => {
          ...
          if (outflow.currency === 'USD') {
            totalOutflowsUSD += montant
          } else {
            totalOutflowsCDF += montant
          }
        })
      }
      ...
      // Calculer les soldes disponibles
      const availableCDF = Math.max(0, totalMontantCDF - totalOutflowsCDF)
      const availableUSD = Math.max(0, totalMontantUSD - totalOutflowsUSD)
      const netBalanceCDF = totalMontantCDF - totalOutflowsCDF
      const netBalanceUSD = totalMontantUSD - totalOutflowsUSD
      ...
      <div className="max-w-7xl mx-auto">
       ...
       {activeTab !== 'categories' && (
        <div className="mb-6">
          ...
          {/* Grille de stats - Style minimaliste */}
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            {/* Total des ventes */}
            <div className="bg-white rounded-lg border border-gray-200 p-3">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-gray-500 font-medium">Total des ventes</p>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-3">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-gray-500 font-medium">Total en Francs</p>
                  <p className="text-lg font-bold text-green-600 mt-1">
                    {loadingStats ? '...' : formatCurrency(stats.totalMontantCDF, 'CDF')}
                  </p>
                </div>
                <div className="p-2 bg-green-50 rounded-full">
                  <span className="text-sm font-bold text-green-600">FC</span>
                </div>
              </div>
              <div className="mt-2 text-xs text-gray-500">
                {stats.ventesCDF} vente{stats.ventesCDF !== 1 ? 's' : ''}
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-3">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-gray-500 font-medium">Total en Dollars</p>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-3">
                ...
                <div>
                  ...
                  <div className="mt-1 space-y-1">
                    <p className="text-sm font-semibold text-red-600">
                      {loadingStats ? '...' : formatCurrency(stats.totalOutflowsCDF, 'CDF')}
                    </p>
                    <p className="text-sm font-semibold text-red-600">
                      {loadingStats ? '...' : formatCurrency(stats.totalOutflowsUSD, 'USD')}
                    </p>
                  </div>
                </div>
              ...
              <div className="mt-2 text-xs text-gray-500">
                {todaysOutflows.length} sortie{todaysOutflows.length !== 1 ? 's' : ''}
              </div>
            </div>
                ...
                <div>
                  ...
                  <div className="mt-1 space-y-1">
                    <p className={`text-sm font-semibold ${
                      stats.availableCDF >= 0 ? 'text-gray-900' : 'text-red-600'
                    }`}>
                      {loadingStats ? '...' : formatCurrency(stats.availableCDF, 'CDF')}
                    </p>
                    <p className={`text-sm font-semibold ${
                      stats.availableUSD >= 0 ? 'text-blue-600' : 'text-red-600'
                    }`}>
                      {loadingStats ? '...' : formatCurrency(stats.availableUSD, 'USD')}
                    </p>
                  </div>
                </div>
