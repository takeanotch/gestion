...
export default function RapportsFinanciersPage() {
  ...
  // Catégories de dépenses
  const categories = [
    { value: 'Transport', label: t('transport'), color: '#8884d8' },
    { value: 'Fournitures', label: t('supplies'), color: '#82ca9d' },
    { value: 'Salaires', label: t('salaries'), color: '#ffc658' },
    { value: 'Loyer', label: t('rent'), color: '#ff8042' },
    { value: 'Électricité', label: t('electricity'), color: '#0088fe' },
    { value: 'Internet', label: t('internet'), color: '#00c49f' },
    { value: 'Marketing', label: t('marketing'), color: '#ffbb28' },
    { value: 'Divers', label: t('other'), color: '#ff6b6b' }
  ]
  ...
  const getCurrentUser = async () => {
    try {
      ...
      console.error(language === 'fr' ? 'Erreur récupération utilisateur:' : 'Error fetching user:', error)
      return null
    }
  }
  ...
  const loadSales = async () => {
    try {
      ...
      console.error(language === 'fr' ? 'Erreur chargement ventes:' : 'Error loading sales:', error)
    }
  }
  ...
  const loadDailyReports = async () => {
    try {
      ...
      console.error(language === 'fr' ? 'Erreur chargement rapports:' : 'Error loading reports:', error)
    }
  }
  ...
  const loadOutflows = async () => {
    try {
      ...
      console.error(language === 'fr' ? 'Erreur chargement sorties:' : 'Error loading outflows:', error)
    }
  }
  ...
  const loadDailyStats = async () => {
    try {
      ...
      console.error(language === 'fr' ? 'Erreur chargement stats:' : 'Error loading stats:', error)
    }
  }
  ...
  const handleCreateOutflow = async () => {
    ...
    const currentUser = await getCurrentUser()
    if (!currentUser) {
      alert(language === 'fr' ? 'Session expirée, veuillez vous reconnecter' : 'Session expired, please login again')
      router.push('/auth/login')
      return
    }
    ...
    try {
      ...
      alert(language === 'fr' ? 'Sortie de caisse enregistrée avec succès' : 'Outflow recorded successfully')
    } catch (error) {
      console.error(language === 'fr' ? 'Erreur création sortie:' : 'Error creating outflow:', error)
      alert(language === 'fr' ? 'Erreur lors de l\'enregistrement de la sortie' : 'Error recording outflow')
    }
  }
  ...
  const chartData = reports.slice(0, 7).reverse().map(report => ({
    date: new Date(report.report_date).toLocaleDateString(language === 'fr' ? 'fr-FR' : 'en-US', { 
      day: 'numeric', 
      month: 'short' 
    }),
    recetteCDF: parseFloat(report.total_receipt_cdf) || 0,
  ...
  const handleExport = async () => {
    try {
      ...
      console.error(language === 'fr' ? 'Erreur export:' : 'Export error:', error)
    }
  }
  ...
  const getDateRangeText = () => {
    switch (filters.dateRange) {
      ...
      case 'thisMonth':
        return t('thisMonth')
      case 'custom':
        return `${t('period')} ${filters.startDate} ${t('to')} ${filters.endDate}`
      default:
        return t('today')
    }
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <div className="flex flex-wrap gap-3 mt-2">
            ...
            <div className="flex items-center space-x-1 text-sm">
              <DollarSign className="w-4 h-4 text-gray-500" />
              <span className="text-gray-500">{t('rate')}: 1$ = {saleConfig.currency_rate?.toLocaleString(language === 'fr' ? 'fr-FR' : 'en-US') || '2,300'} FC</span>
            </div>
          </div>
        </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              {chartType === 'bar' ? (
                <BarChart data={chartData}>
                  ...
                  <YAxis stroke="#6b7280" fontSize={12} />
                  <Tooltip
                    formatter={(value) => [formatCurrency(value, 'CDF'), t('amount')]}
                    labelFormatter={(label) => `${t('date')}: ${label}`}
                  />
                  <Legend />
                  <Bar dataKey="recetteCDF" name={t('revenueCDF')} fill="#10b981" />
                  <Bar dataKey="depenseCDF" name={t('expensesCDF')} fill="#ef4444" />
                  <Bar dataKey="soldeCDF" name={t('balanceCDF')} fill="#3b82f6" />
                </BarChart>
              ) : (
                <LineChart data={chartData}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" />
                  ...
                  />
                  <Legend />
                  <Line type="monotone" dataKey="recetteCDF" name={t('revenueCDF')} stroke="#10b981" strokeWidth={2} />
                  <Line type="monotone" dataKey="depenseCDF" name={t('expensesCDF')} stroke="#ef4444" strokeWidth={2} />
                  <Line type="monotone" dataKey="soldeCDF" name={t('balanceCDF')} stroke="#3b82f6" strokeWidth={2} />
                </LineChart>
              )}
            </ResponsiveContainer>
          </div>
            ...
            {getCategoryData(activeCurrencyTab).length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip
                    formatter={(value) => [formatCurrency(value, activeCurrencyTab), t('amount')]}
                  />
                  <Legend />
                </PieChart>
              </ResponsiveContainer>
              ...
              <div className="flex flex-col items-center justify-center h-full text-gray-500">
                <PieChartIcon className="w-12 h-12 mb-2" />
                <p>{t('noExpensesInCurrency', { currency: activeCurrencyTab })}</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 mb-6">
        <div className="p-4 border-b border-gray-200">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <h3 className="text-lg font-semibold text-gray-900 flex items-center">
              <ArrowDownRight className="w-5 h-5 mr-2 text-red-600" />
              {t('cashOutflows')} ({outflows.length})
            </h3>
        ...
        {outflows.filter(o => o.currency === activeCurrencyTab).length > 0 ? (
              ...
              <tbody className="divide-y divide-gray-200">
                {outflows
                  ...
                  <tr key={outflow.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {new Date(outflow.date).toLocaleDateString(language === 'fr' ? 'fr-FR' : 'en-US', {
                          day: '2-digit',
                          month: '2-digit',
                          year: 'numeric'
                        })}
                    ...
                    {activeCurrencyTab === 'USD' && outflow.currency_rate && (
                      <td className="px-4 py-3">
                        <div className="text-xs text-gray-700">
                          1$ = {outflow.currency_rate.toLocaleString(language === 'fr' ? 'fr-FR' : 'en-US')} FC
                        </div>
                      </td>
                    )}
                    {activeCurrencyTab === 'USD' && !outflow.currency_rate && (
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="text-sm text-gray-500">
                        {new Date(outflow.created_at).toLocaleTimeString(language === 'fr' ? 'fr-FR' : 'en-US', {
                          hour: '2-digit',
                          minute: '2-digit'
                        })}
                      </div>
          ...
          <div className="p-8 text-center">
            <ArrowDownRight className="w-12 h-12 text-gray-400 mx-auto mb-3" />
            <h4 className="text-gray-900 font-medium mb-1">
              {t('noOutflowsInCurrency', { currency: activeCurrencyTab })}
            </h4>
            <p className="text-gray-500 text-sm mb-4">
              {t('recordOutflowInCurrency', { currency: activeCurrencyTab })}
            </p>
            <button
              onClick={() => {
                setNewOutflow({
            ...
            >
              <ArrowDownRight className="w-4 h-4 mr-2" />
              {t('newOutflowInCurrency', { currency: activeCurrencyTab })}
            </button>
          </div>
        )}
      </div>
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  {newOutflow.currency === 'USD' && (
                    <p className="text-xs text-gray-500 mt-1">
                      {t('rate')}: 1$ = {saleConfig.currency_rate?.toLocaleString(language === 'fr' ? 'fr-FR' : 'en-US')} FC
                    </p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows="3"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-1 focus:ring-red-600 focus:border-red-600 text-sm"
                    placeholder={t('outflowReasonPlaceholder')}
                  />
                </div>
