...
export default function CategoryManager() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      setMessage({ type: 'error', text: t('categoriesLoadError') })
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleAddCategory = async (e) => {
    ...
    // Vérifier si la catégorie existe déjà
    if (categories.some(cat => cat.name.toLowerCase() === newCategory.toLowerCase().trim())) {
      setMessage({ type: 'error', text: t('categoryAlreadyExists') })
      return
    }
    ...
    try {
      ...
      setMessage({ type: 'error', text: t('categoryAddError') })
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-md border border-gray-200">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-medium text-gray-900">{category.name}</h3>
                          <p className="text-xs text-gray-500">
                            {t('createdOn')} {new Date(category.created_at).toLocaleDateString()}
                          </p>
                        </div>
                      </div>
          ...
          {categories.length > 0 && (
            <div className="px-6 py-3 border-t border-gray-200 bg-gray-50">
              <div className="flex items-center justify-between text-xs text-gray-500">
                <span>
                  {filteredCategories.length} {t('categoryFound')}{filteredCategories.length !== 1 ? 's' : ''}
                </span>
                <span className="flex items-center gap-1">
                  <Folder className="w-3 h-3" />
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div className="flex items-start gap-3">
          <AlertCircle className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" />
          <div>
            <h4 className="text-sm font-medium text-blue-900">{t('information')}</h4>
            <p className="text-sm text-blue-700 mt-1">
              {t('categoryDeleteInfo')}
            </p>
          </div>
        </div>
      </div>
