...
const saleItemsWithNames = await Promise.all(
  cart.map(async (item) => {
    ...
    // RETOURNEZ LES DEUX NOMS DE PROPRIÉTÉS
    ...
    return {
      name: product?.name || item.product_name,
      quantity: item.quantity,
      unit_price: unitPriceConverted,     // Pour FactureGenerator
      unitPrice: unitPriceConverted,      // Alias pour compatibilité
      total: totalConverted,              // Pour FactureGenerator
      total_price: totalConverted         // Alias pour compatibilité
    }
  })
)
