...
export default function AddProductForm() {
  ...
  const loadCategories = async () => {
    try {
      console.log(t('loadingCategories'));
      const { data, error } = await supabase
        .from('categories')
        .select('id, name')
        .order('name');
      ...
      if (error) {
        ...
        setErrorMessage(`${t('categoryLoadError')}: ${error.message}`);
        return;
      }
      
      console.log(`${data?.length || 0} ${t('categoriesLoaded')}`);
      setCategories(data || []);
    } catch (error) {
      console.error(t('categoryLoadException'), error);
      setErrorMessage(`${t('exception')}: ${error.message}`);
    }
  };
  ...
  const generateSku = () => {
    try {
      ...
      console.error(t('skuGenerationError'), error);
      // Fallback simple avec timestamp
      const timestamp = Date.now().toString().slice(-4);
      return `CAT-PRO-PIECE-${timestamp}`;
    }
  ...
  const uploadProductImage = async (productId, categoryName) => {
    ...
    try {
      ...
      console.log(t('uploadingImage'), { filePath, fileSize: imageFile.size });
      ...
      if (uploadError) {
        console.error(t('imageUploadError'), uploadError);
        return null;
      }
      ...
      console.log(t('imageUploaded'), publicUrl);
      return { path: filePath, url: publicUrl };
    } catch (error) {
      console.error(t('imageUploadException'), error);
      return null;
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.retail_price || parseFloat(formData.retail_price) <= 0) {
        throw new Error(t('retailPriceRequired'));
      }
      
      if (!formData.wholesale_price || parseFloat(formData.wholesale_price) <= 0) {
        throw new Error(t('wholesalePriceRequired'));
      }
      
      if (parseFloat(formData.wholesale_price) > parseFloat(formData.retail_price)) {
        throw new Error(t('wholesaleHigherThanRetail'));
      }

      // Générer le SKU
      const sku = generateSku();
      console.log(t('skuGenerated'), sku);
      console.log(t('originalRetailPrice'), formData.retail_price);
      ...
      console.log(t('productData'), productData);
      ...
      if (productError) {
        console.error(t('productCreationError'), productError);
        throw new Error(`${t('productCreationError')}: ${productError.message}`);
      }

      console.log(t('productCreated'), product);
      ...
      if (imageFile && product.category_id) {
        const category = categories.find(c => c.id === product.category_id);
        if (category) {
          console.log(t('startingImageUpload'), category.name);
          imageData = await uploadProductImage(product.id, category.name);
          
          if (imageData) {
            console.log(t('updatingProductImage'));
            const { error: updateError } = await supabase
              .from('products')
              .update({
                image_url: imageData.url,
            ...
            if (updateError) {
              console.error(t('imageUpdateError'), updateError);
              // On continue même si l'image échoue
            }
          }
        }
      ...
      console.log(t('stockData'), stockData);
      ...
      if (stockError) {
        console.error(t('stockCreationError'), stockError);
        // On essaie de supprimer le produit si le stock échoue
        await supabase.from('products').delete().eq('id', product.id);
        throw new Error(`${t('stockCreationError')}: ${stockError.message}`);
      }

      console.log(t('productSuccess'));
      alert(`${t('productCreatedSuccess')}\n${t('sku')}: ${sku}\n${t('name')}: ${product.name}\n${t('price')}: ${product.retail_price} ${product.currency}`);
      ...
      console.error(t('completeError'), error);
      setErrorMessage(error.message);
  ...
  const getSkuExamples = () => {
    ...
    return (
      <div className="text-xs text-gray-600 mt-1">
        <p>{t('priceWillGive')} "{formData.retail_price}" {t('willGenerate')}: <span className="font-mono">{retailPriceCode}</span></p>
        <p className="mt-1">{t('skuExamples')}</p>
      </div>
    );
  };
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md">
      <h2 className="text-2xl font-bold mb-6">{t('addNewProduct')}</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {previewSku && (
          <div className="bg-blue-50 p-3 rounded border border-blue-200">
            <p className="text-sm font-medium text-blue-800">{t('generatedSku')}</p>
            <p className="text-lg font-bold text-blue-700 font-mono">{previewSku}</p>
            <p className="text-xs text-blue-600 mt-1">
              {t('skuFormat')}
            </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
              ...
              <option value="BOUTEILLE">{t('unitBottle')}</option>
              <option value="SACHET">{t('unitSachet')}</option>
              <option value="CARTON">{t('unitCarton')}</option>
            </select>
          </div>

          {/* Quantité */}
          <div>
            <label className="block text-sm font-medium mb-1">{t('initialQuantity')}</label>
            <input
              type="number"
              value={formData.quantity}
              onChange={(e) => setFormData({...formData, quantity: e.target.value})}
        ...
        <div>
          ...
          <input
            ...
            className="w-full p-2 border rounded"
            min="1"
            placeholder={t('stockAlertPlaceholder')}
          />
        </div>
        ...
        <div className="pt-4">
          ...
          >
            {loading ? t('creatingProduct') : t('addProduct')}
          </button>
        </div>
      </form>
    </div>
